...
extends BlockCrops {
    ...
    public boolean onBlockActivated(World worldIn, int x, int y, int z, EntityPlayer playerIn, int side, float hitX, float hitY, float hitZ) {
        ...
        if (itemstack != null) {
            if (itemstack.getItem() == MyItems.sickle) {
                ...
                if (age >= 7) {
                    worldIn.setBlock(x, y, z, MyBlocks.rices, age - 4, 2);
                    EntityItem entityItem = new EntityItem(worldIn, (double)x + 0.5, (double)y + 0.5, (double)z + 0.5, new ItemStack(MyItems.rice, 1 + rand.nextInt(3)));
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        for (int i = 0; i < amount; ++i) {
            switch (metadata) {
                case 7: {
                    result.add(new ItemStack(MyItems.rice));
                }
            }
        }
        if (metadata >= 5) {
