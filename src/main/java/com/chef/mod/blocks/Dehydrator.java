...
extends BlockContainer {
    ...
    public Dehydrator(boolean isBurning) {
        super(Material.rock);
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        this.blockIcon = iconRegister.registerIcon("chef:dehydrator_side");
        this.iconFront = this.isBurning ? iconRegister.registerIcon("chef:dehydrator_front_on") : iconRegister.registerIcon("chef:dehydrator_front_off");
        this.iconTop = iconRegister.registerIcon("chef:dehydrator_top");
        this.iconBottom = iconRegister.registerIcon("minecraft:hardened_clay");
    }

    @SideOnly(value=Side.CLIENT)
    public IIcon getIcon(int side, int metadata) {
        return metadata == 0 && side == 3 ? this.iconFront : (side == 1 ? this.iconTop : (side == 0 ? this.iconBottom : (side == metadata ? this.iconFront : this.blockIcon)));
    }

    public Item getItemDropped(int i, Random random, int j) {
        return Item.getItemFromBlock((Block)MyBlocks.dehydrator);
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        if (!world.isRemote) {
            FMLNetworkHandler.openGui((EntityPlayer)player, (Object)Chef.instance, (int)5, (World)world, (int)x, (int)y, (int)z);
        }
        return true;
    }
    ...
    public static void updateBlockState(boolean isBurning, World worldObj, int xCoord, int yCoord, int zCoord) {
        ...
        keepInventory = true;
        if (isBurning) {
            worldObj.setBlock(xCoord, yCoord, zCoord, MyBlocks.dehydrator_on);
        } else {
            worldObj.setBlock(xCoord, yCoord, zCoord, MyBlocks.dehydrator);
        }
        keepInventory = false;
    ...
    public void breakBlock(World world, int x, int y, int z, Block oldblock, int oldMetadata) {
        TileEntityDehydrator tileentity;
        if (!keepInventory && (tileentity = (TileEntityDehydrator)world.getTileEntity(x, y, z)) != null) {
